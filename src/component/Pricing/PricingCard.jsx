...
const PricingCard = ({ pricingCard }) => {
    return (
        <div>
            <div className={`h-full flex flex-col w-96 ${pricingCard.tag=="Most Popular"?"bg-linear-to-b from-[#4F39F6] to-[#9514FA]":"bg-base-200 "} shadow-sm rounded-2xl relative`}>
                <div className="  p-4 text-left">
                    <span className={`bg-amber-200 p-1 ${pricingCard.tag=="Most Popular"?"absolute":"hidden"} text-amber-600 rounded-full w-25  text-center text-[1px]  -top-4 left-38  `}>{pricingCard.tag}</span>
                    <div >
                    ...
                    <h1 className={`text-[40px] ${pricingCard.tag=="Most Popular"?"text-white":"text-[#627382]"} font-bold my-6`}>${pricingCard.price}/<span className="text-[16px] text-[#627382]">{pricingCard.period}</span></h1>
